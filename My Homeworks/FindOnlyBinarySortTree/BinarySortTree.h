...
void BinarySortTree<ElemType>::SuperFind(const ElemType &e)
{
	...
	if(p == NULL)
	{
		...
		{
			if(e < q->data)
				p = q->leftChild = new BinTreeNode<ElemType>(e,NULL,NULL);
			else
				p = q->rightChild = new BinTreeNode<ElemType>(e,NULL,NULL);
		}
		else
		{
			p = root = new BinTreeNode<ElemType>(e,NULL,NULL);
		}
	}
	++p->count;
}
