...
{
	int start ,end;							//added to help showing critical path
	...
	while (!q.IsEmpty())	{	// 队列非空
		...
		count++;				// 对顶点进行记数
		if(count == 1)			//added to help showing critical path
			start = u;
	...
	system("pause");
	cout << "显示关键路径:" << endl;
	g.ShowCriticalPath(start, end);//added to help showing critical path
